...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $c_pemberkatan_nikah = '[';
        for ($i=1; $i<=12; $i++){
            $penyerahan_anak = PenyerahanAnak::whereRaw(DB::raw('MONTH(created_at)='.$i))->count();
            $c_penyerahan_anak.=$penyerahan_anak.',';

            $baptisan = Baptisan::whereRaw(DB::raw('MONTH(created_at)='.$i))->count();
            $c_baptisan.=$baptisan.',';

            $pemberkatan_nikah = PemberkatanNikah::whereRaw(DB::raw('MONTH(created_at)='.$i))->count();
