        ...
        <div class="row">
            <div class="col-md-12">
                <div class="pull-right">
                    <a class="btn aqua" href="{{ route('member.penyerahananak.create') }}"><i class="fa fa-plus"></i> Tambah Data</a>
                </div>
            </div>
        </div>
        <div class="row">
                                ...
                                <div class="col-lg-12" >
                                    <table id="example7" class="display nowrap table  responsive nowrap table-bordered">
                                        ...
                                        @foreach($model as $row)
                                            <tr>
                                                <td>{{ $no }}</td>
                                                <td><img src="{{ url('assets/anak/'.$row->img) }}" width="150" height="200"> </td>
                                                <td>{{ $row->nama_anak }}</td>
                                                <td>{{ $row->anak_ke }}</td>
                                                <td>{{ $row->pendeta }}</td>
                                                <td>{{ $row->getStatus() }}</td>
                                                <td>
                                                    <a href="{{ route('member.penyerahananak.show',$row->id) }}" class="btn aqua  btn-xs">detail</a>
                                                    <!-- <a href="{{ route('admin.users.member.edit',$row->id) }}" class="btn yellow  btn-xs">edit</a> -->
                                                </td>
                                            </tr>
                                            <?php $no++; ?>
                                        @endforeach
